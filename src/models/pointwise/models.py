...
POINTWISE_FACTORY = {"linear": lambda input_size, output_size, layers, activation, args: LinearNetwork(input_size, output_size, layers, activation, args),
                     "conv": lambda input_size, output_size, layers, activation, args: ConvNetwork(input_size, output_size, layers, activation, args)}
...
class ConvNetwork(nn.Module):
    def __init__(self, input_size, output_size, layers, activation, args):
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=20, kernel_size=5, stride=1, padding=2,  bias=False)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv2 = nn.Conv2d(in_channels=20, out_channels=50, kernel_size=5, stride=1, padding=2,  bias=False)
