...
parser.add_argument('--task', type=str, default='regression',
                    help='the main task; defines loss')
parser.add_argument('--model_type', type=str, default='stochastic',
                    help='whether the model is pointwise or stochastic')
parser.add_argument('--model', type=str, default='linear_bbb',
                    help='the model that we want to train')

parser.add_argument('--learning_rate', type=float, default=0.005,
                    help='init learning rate')
parser.add_argument('--loss_scaling', type=str, default='batch',
...
parser.add_argument('--dataset_size', type=float, default=1.0,
                    help='portion of the whole training data')
parser.add_argument('--valid_portion', type=float, default=0.2,
                    help='portion of training data')

parser.add_argument('--gamma_start_epoch', type=int, default=10,
                    help='portion of training data')
parser.add_argument('--gamma_end_epoch', type=int, default=500,
                    help='portion of training data')
parser.add_argument('--gamma_start_value', type=float, default=0.,
                    help='portion of training data')
parser.add_argument('--gamma_end_value', type=float, default=.0005,
                    help='portion of training data')
...
parser.add_argument('--activation', type=str, default='relu',
                    help='specify the activation')
parser.add_argument('--input_size', nargs='+', default=[10],
                    help='input size')
...
def main():
    ...
    (best_error, train_time, val_time) = train.train_loop(train_loader,
            valid_loader, logging, writer)

    logging.info('## Finished training, the best observed validation error: {}, total training time: {}, total validation time: {} ##'.format(best_error,
                 timedelta(seconds=train_time),
    ...
    with torch.no_grad():
        args.samples = 1000
        model = model_temp(args.input_size, args.output_size,
                           args.layers, args.activation, args)
