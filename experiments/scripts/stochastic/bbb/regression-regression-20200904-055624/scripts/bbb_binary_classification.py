...
from experiments.utils import plot_binary_uncertainty
...
parser = argparse.ArgumentParser("stochastic_mc_binary_classifier")

parser.add_argument('--task', type=str, default='binary_classification', help='the main task; defines loss')
parser.add_argument('--model_type', type=str, default='stochastic', help='whether the model is pointwise or stochastic')
parser.add_argument('--model', type=str, default='linear_bbb', help='the model that we want to train')
...
parser.add_argument('--dataset', type=str, default='binary_classification',
                    help='dataset')
...
parser.add_argument('--gamma_start_value', type=float,
                    default=0.0, help='portion of training data')
parser.add_argument('--gamma_end_value', type=float,
                    default=7.5, help='portion of training data')
...
parser.add_argument('--layers', nargs='+',
                    default=[10], help='num of init channels')
...
def main():
  ...
  with torch.no_grad():
    ...
    model.eval()
    plot_binary_uncertainty(model, PLT, train_loader, args)
